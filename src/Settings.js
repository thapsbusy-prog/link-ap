    ...
    return (
      <div style={{ background: COLORS.bg, minHeight: "100%" }}>
        <div style={{
          ...
          <button
            onClick={() => setShowFeatureStudio(false)}
            style={{ background: "none", border: "none", color: COLORS.text, cursor: "pointer", padding: 0, fontSize: 22, lineHeight: 1 }}
          >‹</button>
          <div style={{ fontWeight: 700, color: COLORS.text, fontSize: 16 }}>Feature Cards</div>
        </div>
        <FeatureCardGenerator />
      </div>
    );
  }
      ...
      {isAdmin && (
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ background: COLORS.card, border: `1px solid ${COLORS.border}`, borderRadius: 12, overflow: "hidden" }}>
            ...
            <button
              onClick={() => setShowFeatureStudio(true)}
              style={{
                width: "100%", background: "none", border: "none", cursor: "pointer",
                padding: "14px 16px", display: "flex", alignItems: "center", gap: 12,
                borderTop: `1px solid ${COLORS.border}`,
