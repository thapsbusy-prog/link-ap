...
- `src/Pulse.js` — `TIERS` array (`basic` → `/api/pulse`, 5-day refresh; `strategic` → `/api/pulse-strategic`, 7-day refresh) drives a tier-picker pill row rendered above the countdown chip. Icon decision: pills use plain text only (no SVG/emoji), matching `Tools.js`'s `SUBTABS` pill pattern (Founders Hub / Freelancer Kit / Growth Lab) — that's the closest existing precedent for an in-tab tier picker, confirmed with the user before building.
- `usePulseFeed(firebaseUser, path)` hook (in `Pulse.js`) holds independent `ideas`/`loading`/`error`/`generatedAt`/`stale` state per tier so switching tabs doesn't refetch a tier already loaded this session; `activeTier` state defaults to `"basic"`. A `useEffect` lazily fetches a tier's feed the first time it's selected (`ideas === null && !loading && !error`).
- `api/pulse-strategic.js` (new file) — mirrors `api/pulse.js` exactly in structure (GET, Firebase auth verification, Firestore cache, serve-stale-on-failure, `claude-sonnet-4-6`), but: cache doc is `aiTrends/strategic` (separate from `aiTrends/latest`); TTL is 7 days/168 hours; prompt is reframed for capital-intensive ideas (startupCost ranges roughly R20,000–R250,000+, funding/credit assumed rather than bootstrapping). **Not plan-gated yet** — has a `// TODO: gate via getUserPlan when this tier is monetized` comment positioned where the `getUserPlan` check would go, matching the pattern in `api/tools/daily-angle-generate.js`.
...
| Strategic Ideas tier | ✅ Shipped (24 June 2026) | `src/Pulse.js` tier picker + `api/pulse-strategic.js`; daily Vercel cron at 06:00 UTC (no-ops until cache expires); 7-day Firestore cache at `aiTrends/strategic`; 5 capital-intensive SA business-idea playbooks per batch with `fundingRoute`/`competitiveMoat`/`scaleTimeline`; not plan-gated yet (TODO comment in route) |
