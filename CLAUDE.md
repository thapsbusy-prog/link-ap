...
- **Founders Hub — Cash Flow Projection (25 June 2026)** — `src/tools/CashFlowProjection.js`: pure client-side, no API, not plan-gated. Two-step flow rather than the usual form→preview: an assumptions form (businessName pre-filled from user.name, currency, start month via `<input type="month">`, startingCash, avgMonthlyRevenue + optional revenueGrowthPct, avgMonthlyExpenses + optional expenseGrowthPct) seeds a **12-month editable builder** (`seedMonths()` applies compounding growth per month) rather than a static read-only result — every month's revenue/expenses can be tapped and overridden directly, with net/balance recomputed live on every keystroke (`computeRows()`, not stored in state — derived each render from `months` + `startingCash`). A "↺ Reset all months to your assumptions" link re-runs `seedMonths()` without re-entering the form. Builder view shows: a 12-bar closing-balance sparkline (amber=positive, red=negative), summary cards (Total Revenue, Total Expenses, Ending Balance, Lowest Point + month), a plain-English explanation (warns and suggests financing if the balance ever goes negative), then the 12 editable month rows. Download PDF (single-page jsPDF table: Month/Revenue/Expenses/Net/Balance rows + totals + explanation) and Share (`sharePdf`) both build fresh off current state so edits are always reflected.
...
| Cash Flow Projection | Founders Hub | ✅ Shipped (25 June 2026) | `src/tools/CashFlowProjection.js`; client-side only; no API; not plan-gated; editable 12-month builder seeded from growth-rate assumptions; live recompute; jsPDF download; Web Share API (incl. WhatsApp via file share) |
