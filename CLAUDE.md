...
- `src/tools/` — **(10 June 2026)** all new tool implementations, one file per tool: `QuoteGenerator.js`, `PaymentChaser.js`, `RunwayCalculator.js`, `DailyAngleGenerator.js` **(20 June 2026)**, `CashFlowProjection.js` **(25 June 2026)**, plus `toolsShared.js` (shared constants, styles, and `sharePdf()` helper). `src/Tools.js` is **frozen for new tool code** — only imports and picker-card wiring may be added there for new tools.
...
- **Founders Hub**, **Freelancer Kit**, and **Growth Lab** all use the tool picker pattern: a list of cards where tapping opens the selected tool with a "← Back" link. Each subtab has its own named component (`FoundersHub`, `FreelancerKit`, `GrowthLab`) added in `src/Tools.js`. **(20 June 2026)** Growth Lab converted from a direct single-tool view to a two-card picker to accommodate the new AI Daily Marketing Angle Generator.
...
- **Growth Lab tools** (2): AI Content Calendar Generator, AI Daily Marketing Angle Generator.
...
- **Growth Lab — AI Daily Marketing Angle Generator (20 June 2026)** — `src/tools/DailyAngleGenerator.js`: helps TikTok creators get a fresh content angle for their product/service without running dry or sounding corporate. Form: productOrService (required), audience (optional), currentVibe (pill select: "Casual & funny" / "Real & raw (no filter)" / "Chill & low-key" / "Hype & energetic"), angleType (pill select, default "Surprise me" plus 7 specific types). Calls `POST /api/tools/daily-angle-generate`. Preview card shows: angleLabel (TODAY'S ANGLE header), whyItWorks (italic muted), hook/opening line (with inline Copy shortcut), 3 beats as numbered talking-point prompts (not a script), caption (with inline Copy shortcut), CTA line. Bottom buttons: Start Over, Give me another angle (regenerate — passes `lastAngleLabel` when `angleType === "Surprise me"` to avoid repeating), Copy Hook, Copy Caption. No PDF. Plan-gated.
- **Founders Hub — Cash Flow Projection (25 June 2026, redesigned same day as a full itemized statement)** — `src/tools/CashFlowProjection.js`: pure client-side, no API, not plan-gated. Two-step flow: a minimal setup form (businessName pre-filled from user.name, currency, start month via `<input type="month">`, startingCash) builds a **12-month cash flow statement** laid out like a real financial statement — a horizontally-scrollable table (sticky first/label column) with one row per user-defined line item and one column per month, not a lump-sum calculator. Users add unlimited named Revenue line items (e.g. "Product Sales") and Expense line items (e.g. "Rent", "Electricity", "Interest") via `+ Add Revenue/Expense Item`, each starting with 12 blank per-month amount cells they fill in directly — no growth-rate formula, every cell is independently editable (flat amounts only, per user decision; no per-item growth %). Below the line items: `Total Revenue` / `Total Expenses` rows (summed per month), then `Net Cash Flow`, `Opening Balance`, `Closing Balance` rows — `computeTotals()` walks the 12 months carrying the closing balance forward as the next month's opening balance, recomputed fresh on every render (not stored in state). Below the table: a 12-bar closing-balance sparkline (amber=positive, red=negative), summary cards (Total Revenue, Total Expenses, Ending Balance, Lowest Point + month), and a plain-English explanation warning if the balance ever goes negative. Download PDF (`buildStatementPDF()` — landscape A4 jsPDF, replicates the same Line Item × Month grid with pagination via `checkPage()` if line items overflow a page) and Share (`sharePdf`) both build fresh off current state.
...
- `api/tools/daily-angle-generate.js` (20 June 2026) — POST; verifies auth; gates via `getUserPlan`; gates via `checkToolLimit` (tool key `"daily-angle"`); sanitizes `productOrService` (cap 200) and `audience` (cap 150); validates `currentVibe` and `angleType` against allowlists; passes optional `lastAngleLabel` to Claude when `angleType === "Surprise me"` to encourage variation; prompts Claude to return `{ angleLabel, whyItWorks, hook, beats: string[3], caption, cta }`; pure generation route — no server-computed overrides.
- `api/_lib/getUserPlan.js` — shared Admin SDK plan-gate utility for all tool routes.
- `api/_lib/checkToolLimit.js` — shared Admin SDK per-user monthly usage limiter (`users/{uid}/private/toolLimits`, Firestore transaction); used by `quote-generate.js`, `payment-chase.js`, and `daily-angle-generate.js`.
...
- `vercel.json` — cron `0 6 * * *` hits `/api/pulse` daily (no-ops until the cache expires — see Business Ideas Feed below).
...
- `src/Pulse.js` — default export `Pulse` component (still mounted on the `"pulse"` tab); header reads "💡 Business Ideas" with subheading "Basic ideas for getting started, strategic ideas for growing with capital" (24 June 2026 — updated for the two-tier picker below); renders `IdeaCard` (expandable, with share) and `SkeletonCard` loading state. A countdown chip (`nextBatchLabel`) shows "🌱 Next batch in N days" or "Fresh today" based on `generatedAt`, with cadence varying per tier. Free for all users — not plan-gated.
- `api/pulse.js` — Vercel serverless GET handler; Firestore cache (`aiTrends/latest`, 30-day TTL — changed from 5 days on 25 June 2026, see Strategic Ideas tier note below); calls Anthropic `claude-sonnet-4-6` to generate 5 business-idea cards for unemployed/aspiring South African entrepreneurs; serves stale cache on generation failure. The daily cron keeps hitting the route but only regenerates once the cache is older than 30 days.
...
- Collapsed `IdeaCard` shows emoji + title, category pill, startup-cost chip, and the first sentence of `whyInDemand`; expanded view shows all six labelled sections (What it is / Why it's in demand / Where the market is / How to find clients / How to scale / How to start as a step list), plus three additional labelled sections (Funding route / Competitive moat / 12/24-month scale timeline) rendered only when those fields are present on the idea object — i.e. only for Strategic-tier ideas, Basic ideas are unaffected.
...
**Strategic Ideas tier (24 June 2026)**
- Added a second tier to the Business Ideas feed for people with capital (savings, retrenchment payout, loan, investor backing) wanting bigger, defensible plays rather than low-capital side hustles. The original feed is now labelled "Basic Ideas"; the new one "Strategic Ideas".
- `src/Pulse.js` — `TIERS` array (`basic` → `/api/pulse`; `strategic` → `/api/pulse-strategic`; both `refreshDays: 30` as of 25 June 2026 — originally 5-day/7-day, changed to a shared monthly cadence so infrequent visitors and brand-new users don't miss a batch) drives a tier-picker pill row rendered above the countdown chip. Icon decision: pills use plain text only (no SVG/emoji), matching `Tools.js`'s `SUBTABS` pill pattern (Founders Hub / Freelancer Kit / Growth Lab) — that's the closest existing precedent for an in-tab tier picker, confirmed with the user before building.
- `usePulseFeed(firebaseUser, path)` hook (in `Pulse.js`) holds independent `ideas`/`loading`/`error`/`generatedAt`/`stale` state per tier so switching tabs doesn't refetch a tier already loaded this session; `activeTier` state defaults to `"basic"`. A `useEffect` lazily fetches a tier's feed the first time it's selected (`ideas === null && !loading && !error`). A second `useEffect` (`slowNotice` state) surfaces a "Still generating — a fresh batch can take up to a minute. Hang tight." message under the skeleton cards if a fetch is still loading after 6 seconds, since a cold/uncached generation takes ~90-115s.
- `api/pulse-strategic.js` (new file) — mirrors `api/pulse.js` exactly in structure (GET, Firebase auth verification, Firestore cache, serve-stale-on-failure, `claude-sonnet-4-6`), but: cache doc is `aiTrends/strategic` (separate from `aiTrends/latest`); TTL is 30 days (matches Basic, see above); prompt is reframed for capital-intensive ideas (startupCost ranges roughly R20,000–R250,000+, funding/credit assumed rather than bootstrapping). **Not plan-gated yet** — has a `// TODO: gate via getUserPlan when this tier is monetized` comment positioned where the `getUserPlan` check would go, matching the pattern in `api/tools/daily-angle-generate.js`.
- `vercel.json` — added a second cron entry, `{ "path": "/api/pulse-strategic", "schedule": "0 6 * * *" }`, alongside the existing `/api/pulse` entry; same daily no-op-until-stale pattern.
- Strategic idea schema extends the Basic schema with three new fields: `fundingRoute` (specific SA funding mechanism tied to that idea — SEFA, bank loan, franchise finance, NYDA, angel/family investment, etc.), `competitiveMoat` (1-2 sentences on what stops a competitor copying the idea within 30 days), `scaleTimeline` (short 12/24-month growth trajectory).
- Unlike Basic Ideas, `category` on a Strategic idea is **free-form** (any industry — logistics, manufacturing, hospitality, construction, real estate, etc.), not restricted to the Basic tier's fixed 5-category list. The only filter is scalable + in-demand + meaningfully more capital-intensive than a Basic idea. `IdeaCard`'s `CATEGORY_COLORS` lookup already falls back to `COLORS.accent` for unrecognized category strings, so no UI change was needed to support this.
- Fixed a launch-day bug: the initial prompt asked for 13 fields × 5 ideas at `max_tokens: 4000`, which truncated Claude's JSON output mid-array (confirmed via a temporary debug field surfaced in the 500 response, then reverted) and caused every request to fail after ~90s. Raised `max_tokens` to 8000 to give the larger schema room to complete.
- Firestore collection `aiTrends/strategic` — already covered by the existing `match /aiTrends/{docId}` wildcard rule in `firestore.rules` (blocks all client read/write); no rule change was needed.
- `src/FeatureCardGenerator.jsx` — added a `"Strategic Ideas"` key to `FEATURE_CARDS` with 3 persona cards (retrenched worker with a payout, funded founder, small business owner expanding), per the Content Studio feature-card rule.

**Funded Ideas tier (25 June 2026)**
- Added a third tier to the Business Ideas feed, sitting between Basic and Strategic, for people with a modest amount of capital (savings, a small loan, a stokvel/savings-club payout, a small severance) — labelled "Funded Ideas".
- `src/Pulse.js` — `TIERS` array now has three entries in order `basic` → `funded` → `strategic` (`funded` → `/api/pulse-funded`, `refreshDays: 30`, same monthly cadence as the other two tiers). The `feed`/`tier` selection in the `Pulse` component switched from a binary ternary to a `feedsByTier = { basic, funded, strategic }` lookup object to support a third tier; `usePulseFeed` itself is unchanged (still one independent hook instance per tier). Header subheading updated to mention all three tiers.
- `api/pulse-funded.js` (new file) — mirrors `api/pulse-strategic.js` in structure (GET, Firebase auth verification, Firestore cache, serve-stale-on-failure, `claude-sonnet-4-6`, `max_tokens: 8000`), but: cache doc is `aiTrends/funded`; TTL is 30 days; prompt targets ZAR startupCost roughly R15,000–R150,000 — real small businesses with upfront cost (stock, equipment, a vehicle, a small premises) but below Strategic's capital-intensive bar; **not plan-gated** (this tier has no `getUserPlan` TODO comment at all, unlike Strategic — it was scoped from the start as a free-tier extension, matching Basic and the not-yet-monetized Strategic tier).
- Like Strategic Ideas, `category` on a Funded idea is **free-form** (confirmed with the user before building, to stay consistent with Strategic rather than Basic's fixed 5-category list) — no `fundingRoute`/`competitiveMoat`/`scaleTimeline` fields though; those stay exclusive to Strategic. `IdeaCard`'s existing field-presence filtering already only renders those three extra sections when present, so no `IdeaCard` changes were needed.
- `vercel.json` — added a third cron entry, `{ "path": "/api/pulse-funded", "schedule": "0 6 * * *" }`, alongside the existing `/api/pulse` and `/api/pulse-strategic` entries; same daily no-op-until-stale pattern. Note: if the Vercel plan in use caps the number of cron jobs, this third entry may need verifying after deploy.
- Firestore collection `aiTrends/funded` — already covered by the existing `match /aiTrends/{docId}` wildcard rule in `firestore.rules`; no rule change was needed.
- `src/FeatureCardGenerator.jsx` — added a `"Funded Ideas"` key to `FEATURE_CARDS` with 3 persona cards (stokvel/savings-club member, personal loan or small payout recipient, side-hustler ready to reinvest), per the Content Studio feature-card rule.
...
|---------|--------|-------|
| Business Ideas feed (was AI Pulse) | ✅ Shipped (26 May 2026, repurposed 10 June 2026) | `src/Pulse.js` + `api/pulse.js`; daily Vercel cron at 06:00 UTC (no-ops until cache expires); 30-day Firestore cache at `aiTrends/latest` (changed from 5 days on 25 June 2026); 5 SA business-idea playbooks per batch, not plan-gated |
| Funded Ideas tier | ✅ Shipped (25 June 2026) | `src/Pulse.js` tier picker + `api/pulse-funded.js`; daily Vercel cron at 06:00 UTC (no-ops until cache expires); 30-day Firestore cache at `aiTrends/funded`; 5 SA business-idea playbooks per batch sized to R15,000–R150,000 startup capital; free-form category; not plan-gated |
| Strategic Ideas tier | ✅ Shipped (24 June 2026) | `src/Pulse.js` tier picker + `api/pulse-strategic.js`; daily Vercel cron at 06:00 UTC (no-ops until cache expires); 30-day Firestore cache at `aiTrends/strategic` (changed from 7 days on 25 June 2026); 5 capital-intensive SA business-idea playbooks per batch with `fundingRoute`/`competitiveMoat`/`scaleTimeline`; not plan-gated yet (TODO comment in route) |
...
| Cash Flow Projection | Founders Hub | ✅ Shipped (25 June 2026) | `src/tools/CashFlowProjection.js`; client-side only; no API; not plan-gated; itemized 12-month statement — user-defined Revenue/Expense line items (rent, electricity, interest, etc.) in a scrollable Line Item × Month grid; Total/Net/Opening/Closing rows computed live; landscape jsPDF download; Web Share API (incl. WhatsApp via file share) |
...
| AI Daily Marketing Angle Generator | Growth Lab | ✅ Shipped (20 June 2026) | `src/tools/DailyAngleGenerator.js` + `api/tools/daily-angle-generate.js`; Form → Preview; no PDF; Copy Hook / Copy Caption / Regenerate; `lastAngleLabel` passed on regenerate to vary angle when `angleType === "Surprise me"`; plan-gated |
...
| `aiTrends/latest` | Basic Ideas feed batches (30-day TTL, single doc, `{ ideas: [...], generatedAt }`) | Admin SDK only — client read/write blocked in firestore.rules |
| `aiTrends/funded` | Funded Ideas feed batches (30-day TTL, single doc, `{ ideas: [...], generatedAt }`) | Admin SDK only — covered by the same `aiTrends/{docId}` wildcard rule |
| `aiTrends/strategic` | Strategic Ideas feed batches (30-day TTL, single doc, `{ ideas: [...], generatedAt }`) | Admin SDK only — covered by the same `aiTrends/{docId}` wildcard rule |
